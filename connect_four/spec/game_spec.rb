...
module ConnectFour
  describe Game do
    ...
    let (:bob) {Player.new({color: "red", name: "bob"})}
    let (:frank) {Player.new({color: "yellow", name: "frank"})}
    ...
    context "#get_move" do
      it "converts human_move of '1' to 1st column" do
        game = Game.new([bob, frank])
        expect(game.get_move("1")).to eq [0,0]
      end

      it "converts human move of '7' to 7th column" do
        game = Game.new([bob, frank])
        expect(game.get_move("7")).to eq [6, 0]
      end
    end
